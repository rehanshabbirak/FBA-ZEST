      ...
      <div className="flex items-center gap-1.5">
        ...
        <p
          className={`leading-none font-bold tracking-[-0.015em] ${TONE_VALUE[tone]} ${
            metric.value.length > 7 ? "text-[1.0625rem]" : "text-[1.3125rem]"
          }`}
        >
          {metric.value}
        </p>
      </div>
      <p className="mt-1.5 text-[0.6875rem] leading-[1.35] font-medium text-muted">
        {metric.label}
      </p>
    </li>
  );
...
function CaseStudyCard({ study }: { study: CaseStudy }) {
  return (
    <article className="card-lift flex flex-col overflow-hidden rounded-xl border border-line bg-white shadow-card">
      <div className="grid lg:grid-cols-[minmax(0,28.75rem)_minmax(0,1fr)]">
        ...
        <div className="flex flex-col justify-center px-6 py-6 lg:px-7 lg:py-7">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
            <p className="text-[0.6875rem] font-bold tracking-[0.16em] text-teal-500 uppercase">
              Case Study {String(study.caseStudyNumber).padStart(2, "0")}
            </p>
            <span aria-hidden="true" className="h-3 w-[0.0625rem] bg-line" />
            <span className="text-[0.71875rem] font-semibold text-muted">
              {study.industry}
            </span>
          </div>

          <h3 className="mt-2 text-xl leading-[1.25] font-bold tracking-[-0.015em] text-ink lg:text-[1.375rem]">
            {study.cardTitle}
          </h3>

          <p className="mt-2 max-w-[68ch] text-[0.8125rem] leading-[1.6] text-muted">
          ...
          {study.representativePeriod ? (
            <p className="mt-3 text-[0.65625rem] tracking-[0.02em] text-subtle">
              {study.representativePeriod}
            </p>
          ) : null}
        </div>
      ...
      <div className="grid gap-5 border-t border-line bg-surface px-6 py-5 lg:grid-cols-[minmax(0,1.9fr)_minmax(0,1fr)] lg:gap-8 lg:px-7">
        <div className="flex gap-3">
          ...
          />
          <p className="text-[0.78125rem] leading-[1.6] text-muted">
        ...
        <div className="lg:border-l lg:border-line lg:pl-8">
          <p className="text-[0.65625rem] font-bold tracking-[0.14em] text-ink uppercase">
            Services Used
          </p>
          <ul className="mt-2 flex flex-wrap gap-1.5">
            {study.services.map((service) => (
              <li
                key={service}
                className="rounded-full border border-line bg-white px-2.5 py-1 text-[0.6875rem] leading-none text-muted"
              >
                {service}
              </li>
            ))}
    ...
    <section className="bg-white">
      <Container className="py-16 lg:py-20">
        <Reveal className="text-center">
          ...
          <RevealText className="mx-auto mt-4 max-w-4xl text-[1.75rem] leading-[1.15] font-bold tracking-[-0.02em] text-balance text-ink sm:text-[2.125rem] lg:text-[2.375rem]">
          ...
          />
          <p className="mx-auto mt-6 max-w-[68ch] text-[0.9375rem] leading-[1.75] text-muted">
          ...
          <p className="mx-auto mt-4 max-w-[68ch] text-[0.84375rem] leading-[1.7] text-subtle">
